module.exports = (deck, dealer) => {
    ...
    let state = {
        ...
        card: undefined,
        explode: false, // A boolean variable, stores information if the cards went bust or not.
        win: false, // A boolean variable, stores information if the player has won or not.
    };
    return {
        state: state,
         ...
         * Then the function checks if the player has won, of which are also two possibilities
         ...
         * The function first checks if the first two cards dealt are Lucky21.
         * It then returns the boolean variable, state.win, which is by default false
        ...
        /* The highest score the cards can yield without going over 21 (integer) AKA the value of cards currently drawn by the player.
         * The iterates through the cards array, getting the right values from each card. King, Queen and Jack should return 10, and the Ace
         * should return either 1 or 11, so long as the total card value doesn't exceed 21.
         */
        getCardsValue: (game) => {
            var i;
            var x = 0;
            var ace = false;                               //Keeps track of aces in the cards drawn.
            for (i = 0; i < state.cards.length; i++) {
                if(parseInt(state.cards[i]) == 1){
                    ace = true;                           // Here we have found an ace, and we have only added 1 to the final sum.
                    x += 1;
                }
            ...
            if(x + 10 <= 21){
                if(ace){
                    x += 10;                        //Only if I can add 10 to the total without exceeding 21, and provided I have
                }                                   //an ace in the cards drawn, I add the extra 10.
            }
            return x;
        },
        /* The value of the statecard. Returns either an integer or undefined.
         * Is always undefined, unless player has guessed over 21. Then this card
         * is the next card drawn. And the total, value of this card + value of cards on hand,
        ...
        getCardValue: (game) => {
            if(state.card == undefined){
                ...
                if(parseInt(state.card) >= 10){  //Royal people checker
                    ...
                    if(parseInt(state.card) == 1){
                        if((game.getCardsValue(game) + 11) <= 21){  //Checks for the ace, returns 11 if the total doesnt exceed 21.
                            return 11;
                        }
                    }
         ...
         * Pretty straightforward function, just adds getCardsValue and getCardValue together.
         ...
         * The function draws a card from the deck and puts it into the cards-array, AKA the players hand.
         * If the total is 21 now, the player has won, if it went over 21 the player went bust.
         */
        guess21OrUnder: (game) => {
         ...
         * The function draws a card from the deck and puts it into the statecard, which only happens when the
         * player guesses over 21. If the total, this time the players hand + the statecard is over 21, 
